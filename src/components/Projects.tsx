...
// Types
type ProjectStatus = 'Entrada' | 'Briefing' | 'Criação' | 'Aprovação Cliente' | 'PCP' | 'Produção' | 'Qualidade' | 'Expedição' | 'Instalação' | 'Concluído';
...
export function Projects() {
  const [viewMode, setViewMode] = React.useState<'kanban' | 'list' | 'calendar' | 'gantt' | 'timeline' | 'table' | 'dashboard'>('kanban');
  const [projects, setProjects] = React.useState<Project[]>(PROJECTS_DATA);
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 md:space-y-8 animate-in fade-in duration-700 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-zinc-900/20 via-[#050505] to-[#050505]">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
           ...
           <div className="bg-[#0c0c10] border border-white/5 p-1 rounded-xl flex items-center gap-1">
              {[
           ...
           <div className="flex items-center gap-2 w-full sm:w-auto">
              ...
              <Button className="bg-blue-600 text-white hover:bg-blue-700 h-10 font-bold px-4 text-[10px] uppercase tracking-widest shrink-0">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {viewMode === 'kanban' && <KanbanView projects={filteredProjects} onSelect={setSelectedProject} />}
          ...
          {viewMode === 'dashboard' && <ProjectsDashboard projects={filteredProjects} />}
        </motion.div>
      </AnimatePresence>
...
function KanbanView({ projects, onSelect }: { projects: Project[], onSelect: (p: Project) => void }) {
  return (
    <div className="flex gap-4 overflow-x-auto pb-6 -mx-4 px-4 scrollbar-hide snap-x">
      {COLUMNS.map((col) => (
        <div key={col} className="min-w-[300px] w-[300px] flex flex-col gap-3 snap-center">
          <div className="flex items-center justify-between px-2 mb-2">
            <div className="flex items-center gap-3">
              <div className="w-2 h-2 rounded-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.5)]" />
              <h3 className="text-[11px] font-black tracking-[0.2em] text-zinc-300 uppercase italic">{col}</h3>
            </div>
            <Badge variant="outline" className="bg-white/5 border-0 text-[10px] text-zinc-500 font-bold px-2">{projects.filter(p => p.status === col).length}</Badge>
          </div>

          <div className="flex flex-col gap-3 overflow-y-auto max-h-[70vh] p-2 bg-white/[0.01] border border-white/5 rounded-2xl">
            {projects.filter(p => p.status === col).map(p => (
              <div key={p.id}>
                <ProjectCard project={p} onClick={() => onSelect(p)} />
              </div>
            ))}
            <Button variant="ghost" className="w-full h-12 border-2 border-dashed border-white/5 hover:border-white/10 text-zinc-600 hover:text-white text-[10px] font-black tracking-widest uppercase">
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2.5 mb-6 bg-white/[0.02] p-3 rounded-xl border border-white/5">
        ...
        <Progress value={project.progress} className="h-1.5 bg-white/5" indicatorClassName="bg-blue-500" />
      </div>
...
function CalendarView({ projects, onSelect }: { projects: Project[], onSelect: (p: Project) => void }) {
  const [currentMonth, setCurrentMonth] = React.useState(new Date(2026, 4, 1)); // May 2026
  const [calendarMode, setCalendarMode] = React.useState<'month' | 'week' | 'day'>('month');
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
         <div className="flex items-center gap-4">
            <h3 className="text-xl font-black text-white uppercase italic tracking-tight">{format(currentMonth, 'MMMM yyyy', { locale: ptBR })}</h3>
         ...
         <div className="flex gap-2">
            <Button variant="outline" className="bg-white/5 border-white/5 text-[10px] font-bold uppercase tracking-widest h-8 px-4" onClick={() => setCurrentMonth(addDays(currentMonth, -30))}>Anterior</Button>
            <Button variant="outline" className="bg-white/5 border-white/5 text-[10px] font-bold uppercase tracking-widest h-8 px-4" onClick={() => setCurrentMonth(addDays(currentMonth, 30))}>Próximo</Button>
            <Button className="bg-blue-600 text-white h-8 text-[10px] font-bold uppercase tracking-widest px-4">Hoje</Button>
         </div>
      </div>
      ...
      {calendarMode === 'month' ? (
                ...
                <div className="space-y-1.5 h-[100px] overflow-y-auto scrollbar-hide">
                  {dayProjects.map(p => (
                    <div 
                      key={p.id} 
                      onClick={() => onSelect(p)}
                      className="p-1 px-2 rounded bg-blue-600/10 border border-blue-500/20 cursor-pointer group hover:bg-blue-500/20 transition-all"
                    >
        ...
        <div className="bg-[#0c0c10] border border-white/5 rounded-2xl p-12 flex flex-col items-center justify-center text-center space-y-4">
           <div className="w-16 h-16 rounded-full bg-blue-600/10 flex items-center justify-center text-blue-500">
              <CalendarIcon size={32} />
           </div>
           <div>
              <h4 className="text-white font-black uppercase italic tracking-widest mb-2">Visualização {calendarMode === 'week' ? 'Semanal' : 'Diária'}</h4>
              <p className="text-zinc-500 text-xs font-medium max-w-xs">
                A visualização detalhada de {calendarMode === 'week' ? 'semana' : 'dia'} está sendo otimizada pelo núcleo <span className="text-blue-500">KORTECK</span>.
              </p>
           </div>
           <Button variant="outline" className="border-white/10 text-[10px] font-black uppercase tracking-widest h-8" onClick={() => setCalendarMode('month')}>Voltar para Mensal</Button>
        </div>
      )}
    </div>
  );
...
function ProjectsDashboard({ projects }: { projects: Project[] }) {
  const stats = [
          ...
          <Card className="lg:col-span-2 bg-[#0c0c10] border-white/5 p-6 shadow-2xl">
             ...
             <div className="h-[300px] w-full flex items-end gap-2 md:gap-4 pb-8 border-b border-white/5 relative">
                {COLUMNS.map((col, i) => {
                  ...
                  const height = (count / projects.length) * 100 + 10;
                  return (
