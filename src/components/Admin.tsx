  ...
  return (
    <div className="p-4 md:p-8 space-y-8 animate-in fade-in duration-750 max-w-[1600px] mx-auto pb-24">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-8 border-b border-white/5 pb-8">
        ...
        <div className="flex flex-wrap items-center gap-3">
           <Button 
             ...
             className="bg-blue-600 text-white hover:bg-blue-500 font-black h-14 px-8 text-[11px] uppercase tracking-widest shadow-2xl shadow-blue-600/10 transition-all border-0"
           >
             {isAiAuditRunning ? (
           ...
           <Button variant="outline" className="bg-white/5 border-white/10 hover:bg-white/10 text-white h-14 px-8 text-[11px] uppercase font-black tracking-widest">
             Logs Globais
           </Button>
        </div>
      </div>
          ...
          <Card key={i} className="bg-[#0c0c10] border-white/5 relative overflow-hidden group">
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="w-10 h-10 rounded-xl bg-white/[0.02] border border-white/5 flex items-center justify-center group-hover:scale-110 transition-transform">
                  {stat.icon}
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
        <Card className="xl:col-span-8 bg-[#0c0c10] border-white/5 overflow-hidden rounded-3xl">
          <CardHeader className="border-b border-white/5 flex flex-row items-center justify-between p-8 bg-white/[0.01]">
            <div>
              ...
              {[
                ...
                <div key={i} className="p-6 flex items-center justify-between hover:bg-white/[0.01] transition-all group">
                  <div className="flex items-center gap-5">
                    <div className="w-12 h-12 rounded-2xl bg-zinc-900 border border-white/5 flex items-center justify-center text-zinc-500 group-hover:border-blue-500/50 shadow-inner overflow-hidden relative">
                    ...
                    <div>
                      <div className="flex items-center gap-3">
                        <h4 className="text-base font-black text-white italic uppercase tracking-tighter leading-none">{admin.name}</h4>
                        <Badge className={cn(
                          "text-[8px] font-black uppercase h-4 px-2 tracking-widest border-0",
                          admin.status === 'Ativo' ? "bg-emerald-500/10 text-emerald-500" : "bg-white/5 text-zinc-600"
                        )}>
                          {admin.status}
                        </Badge>
                      </div>
                      <p className="text-[10px] font-bold text-zinc-600 tracking-[0.1em] uppercase mt-1.5 leading-none">
                        <span className="text-blue-500/80">{admin.role}</span> // {admin.email}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-10">
                    <div className="hidden sm:block text-right">
                      <p className="text-[11px] font-black text-white italic tracking-tighter uppercase">{admin.level}</p>
                      <p className="text-[9px] font-bold text-zinc-700 uppercase tracking-widest">{admin.lastLogin}</p>
                    </div>
                    <Button variant="ghost" size="icon" className="h-10 w-10 text-zinc-700 hover:text-white transition-colors">
          ...
          <Card className="bg-gradient-to-br from-blue-600/10 to-blue-950/20 border-blue-500/20 overflow-hidden relative p-10 border-2">
            ...
            <div className="relative z-10 space-y-6">
               ...
               <div className="space-y-4">
                  <div className="p-4 bg-black/40 border border-white/5 rounded-2xl flex gap-4">
               ...
               <Button className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black text-[11px] uppercase h-14 tracking-widest shadow-2xl shadow-blue-600/20">
      ...
      <Card className="bg-[#0c0c10] border-white/5 overflow-hidden rounded-3xl">
        <CardHeader className="bg-white/[0.01] border-b border-white/5 p-8 flex flex-row items-center justify-between">
           <div>
